...
def plot_predictions(train_pred, test_pred, N_LABELS, FS=14, nbins=50, train_true=None, test_true=None, axs=None):
    ...
    if n_combos:
        for i, (c1, c2) in enumerate(combos):
            axs[i].hist2d(train_pred[:, c1].numpy(), train_pred[:, c2].numpy(), bins=nbins)
            axs[i].set_xlabel(f'{c1} prediction', fontsize=FS)
            axs[i].set_ylabel(f'{c2} prediction', fontsize=FS)
            axs[i].set_title(f'Predictions distribution train {c1} vs {c2}', fontsize=FS+2)

            axs[i+n_combos].hist2d(test_pred[:, c1].numpy(), test_pred[:, c2].numpy(), bins=nbins)
            axs[i+n_combos].set_xlabel(f'{c1} prediction', fontsize=FS)
            axs[i+n_combos].set_ylabel(f'{c2} prediction', fontsize=FS)
            axs[i+n_combos].set_title(f'Predictions distribution test {c1} vs {c2}', fontsize=FS+2)

    elif train_true is None and test_true is None:
        axs.hist([train_pred, test_pred], bins=nbins, label=['train', 'test'], stacked=True, edgecolor='white')
        axs.legend()
...
def plot_confusion_matrix(y_pred, y_true, classes, FS=14, cmap=plt.cm.Blues, thresh=0.5, ax=None):
    ...
    fmt = 'd'
    cthresh = (cm.max()- cm.min()) / 2.
    for i in range(cm.shape[0]):
        for j in range(cm.shape[1]):
            ax.text(j, i, format(cm[i, j], fmt),
                    ha="center", va="center",
                    color="white" if cm[i, j] > (cm.min()+ cthresh) else "black", fontsize=FS-2)
    return fig, ax
...
def plot_loss(losses, ax, epochs, FS=14, sweep=None):
    ...
    if sweep is not None:
        ...
        if sweep:
            ax.plot([np.array(losses[i:i+steps_per_epoch]).mean() for i in np.arange(0, len(losses), steps_per_epoch)])
...
def plot_feat_en(imp, names, dataset, map_dim=2, labels=[''], FS=14, axs=None, color='tab:blue'):
    unit = int(len(names)**0.5)+2
    max_entropy = np.log(map_dim)
    if axs is None:
        fig, axs = plt.subplots(1, len(labels), figsize=(unit*len(labels)+(len(labels)==1), unit), sharey=True, sharex=True)
    else:
        fig = None
